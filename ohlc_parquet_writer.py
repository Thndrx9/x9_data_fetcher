...
# ---------------------------------------------------------------------------
# Each symbol gets its own table:  quote_RELIANCE, quote_TCS, quote_WIPRO ...
#
# New symbol mid-day:
#   → first enqueue() for that symbol hits _ensure_table()
#   → CREATE TABLE IF NOT EXISTS quote_SYMBOL runs once
...
def _open_db(db_path: Path) -> sqlite3.Connection:
    ...
    print(f"[OHLC_WRITER] DB {action}: {db_path.name}", flush=True)
    return conn


def _load_existing_tables(conn: sqlite3.Connection) -> Set[str]:
    """
    On startup or restart, read which quote_* tables already exist in the DB.
    ...
    if tables:
        print(f"[OHLC_WRITER] found {len(tables)} existing tables in DB", flush=True)
    return tables
...
def _ensure_table(conn: sqlite3.Connection, sym: str, known_tables: Set[str]) -> str:
    ...
    After creation the name is added to known_tables so this runs only once
    per symbol per DB file.
    """
    table = f"quote_{_safe_symbol(sym)}"
    ...
    print(f"[OHLC_WRITER] new table: {table}", flush=True)
    return table


@lru_cache(maxsize=256)
def _insert_sql(table: str) -> str:
    """Cache INSERT SQL per table — 15 columns, built once per symbol."""
    placeholders = ",".join(["?"] * 3)
...
class OhlcParquetWriter:
    ...
    Public interface unchanged:
        writer = OhlcParquetWriter(base_dir)
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._run, name="ohlc-parquet-writer", daemon=True
        )
        self._thread.start()
    ...
    def enqueue(self, symbol: str, row: dict) -> None:
        """Add a quote tick to the write queue. Non-blocking."""
    ...
    def shutdown(self, timeout: float | None = None) -> None:
        ...
        if self._thread.is_alive():
            print(
                "[OHLC_WRITER][ERROR] shutdown timed out before final flush completed",
                flush=True,
            )
    ...
    def _flush(
        ...
        for sym, rows in buffered.items():
            ...
            for row in rows:
                try:
                    ...
                    print(
                        f"[OHLC_WRITER][ERROR] parse failed for {sym}: {exc}",
                        flush=True,
                    )
            ...
            if parsed:
                try:
                    ...
                    print(
                        f"[OHLC_WRITER][ERROR] insert failed for {sym}: {exc}",
                        flush=True,
                    )
        ...
        if any_rows:
            try:
                ...
                print(f"[OHLC_WRITER][ERROR] commit failed: {exc}", flush=True)